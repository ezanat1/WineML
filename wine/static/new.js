    ...
    $('form').on('submit',function(event){
      ...
      .done(function(data){
        if(data.error){
          ...
          let O=$('<div class="card "><div class="card-image waves-effect waves-block waves-light"><img src='+value.url+' style="max-height: 400px; padding: 10px"><a class="btn-floating right btn-small  waves-effect waves-light red addBtn" wineid='+value.id+'><i class="material-icons">add</i></a></div><div class="card-content"><span style="font-size:20px;overflow: hidden">'+value.name+ '</span><p><i class="material-icons">location_on</i>'+value.region+'</p><p><i class="material-icons">attach_money</i>'+value.price+'</p><p><i class="material-icons">restaurant_menu</i>'+value.pairing+'</p></div></div></div>')
          O.appendTo(mainDiv)
